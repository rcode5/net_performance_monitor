...
describe S3Service do
  ...
  describe '.get_contents' do
    it 'retrieves the file contents from s3' do
      ...
      expect(@mock_s3_client).to have_received(:get_object).with(bucket: 'my bucket',
                                                                 key: 'whatever')
    end
  end
  ...
  describe '.put_contents' do
    it 'writes a file to s3' do
      ...
      expect(@mock_s3_client).to have_received(:put_object).with(bucket: 'my bucket',
                                                                 key: 'filename',
                                                                 body: 'whatever')
    end
  end
